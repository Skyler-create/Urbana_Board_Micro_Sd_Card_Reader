...
module sd_spi_file_reader #(
    ...
    parameter [52*8-1:0] FILE_NAME     = "example.RGB", // file name to read, ignore upper and lower case
    ...
    input  logic       rstn,                             // rstn active-low, 1:working, 0:reset
    input  logic       clk,                              // clock 
    // SDcard spi interface
    output logic       spi_ssn, spi_sck, spi_mosi,
    input  logic       spi_miso,
    // status output (optional for user)
    output logic [1:0] card_type,                        // SDv1, SDv2, SDHCv2 or UNKNOWN
    output logic [3:0] card_stat,                        // show the sdcard initialize status
    output logic [1:0] filesystem_type,                  // FAT16, FAT32 or UNKNOWN
    output logic [2:0] filesystem_stat,                  // show the filesystem initialize status
    output logic        file_found,                       // 0=file not found, 1=file found
    // file content data output (sync with clk)
    output logic        outen,                            // when outen=1, a byte of file content is read out from outbyte
...
assign state = filesystem_state;
logic [52*8-1:0] FILE_NAME_UPPER;
...
logic  [31:0] rootdir_sector = 0;       // rootdir sector number (FAT16 only)
logic  [15:0] rootdir_sectorcount = 0;  // (FAT16 only)
...
logic        [2:0] filesystem_state = RESET;
...
logic        [1:0] filesystem = UNASSIGNED,
                 filesystem_parsed;
...
//----------------------------------------------------------------------------------------------------------------------
logic [ 7:0] sector_content [0:511];
...
//----------------------------------------------------------------------------------------------------------------------
logic        is_boot_sector     = ( {sector_content['h1FE],sector_content['h1FF]}==16'h55AA );
logic        is_dbr             =    sector_content[0]==8'hEB || sector_content[0]==8'hE9;
logic [31:0] dbr_sector_no      =   {sector_content['h1C9],sector_content['h1C8],sector_content['h1C7],sector_content['h1C6]};
logic [15:0] bytes_per_sector   =   {sector_content['hC],sector_content['hB]};
logic [ 7:0] sector_per_cluster =    sector_content['hD];
logic [15:0] resv_sectors       =   {sector_content['hF],sector_content['hE]};
logic [ 7:0] number_of_fat      =    sector_content['h10];
logic [15:0] rootdir_itemcount  =   {sector_content['h12],sector_content['h11]};   // root dir item count (FAT16 Only)

logic  [31:0] sectors_per_fat = 0;
logic  [31:0] root_cluster    = 0;
...
// parse root dir
//----------------------------------------------------------------------------------------------------------------------
logic         fready = 1'b0;            // a file is find when fready = 1
logic  [ 7:0] fnamelen = 0;
logic  [15:0] fcluster = 0;
logic  [31:0] fsize = 0;
logic  [ 7:0] fname     [0:51];
logic  [ 7:0] file_name [0:51];
logic         isshort=1'b0, islongok=1'b0, islong=1'b0, longvalid=1'b0;
logic  [ 5:0] longno = 6'h0;
logic  [ 7:0] lastchar = 8'h0;
logic  [ 7:0] fdtnamelen = 8'h0;
logic  [ 7:0] sdtnamelen = 8'h0;
logic  [ 7:0] file_namelen = 8'h0;
logic  [15:0] file_1st_cluster = 16'h0;
logic  [31:0] file_1st_size = 0;
